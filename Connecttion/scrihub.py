...
def _get_available_scihub_urls():
...
class SciHub(object):
    ...
    def __init__(self, limit):
        ...
        self.available_base_url_list = _get_available_scihub_urls()
        self.base_url = self.available_base_url_list[0] + '/'
        self.defaultWait = limit * 100
    ...
    @retry(wait_random_min=defaultWait, wait_random_max=defaultWait, stop_max_attempt_number=10)
    ...
    def fetch(self, identifier):
        ...
        try:
            ...
            logger.info('Failed to fetch pdf with identifier (resolved url) due to request exception.')
            return {
                'err': 'Failed to fetch pdf with identifier (resolved url) due to request exception.'
            }
...
def main(DOI, destination, limit):
